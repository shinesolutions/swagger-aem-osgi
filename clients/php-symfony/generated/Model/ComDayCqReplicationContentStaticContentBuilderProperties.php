<?php
/**
 * ComDayCqReplicationContentStaticContentBuilderProperties
 *
 * PHP version 5
 *
 * @category Class
 * @package  OpenAPI\Server\Model
 * @author   OpenAPI Generator team
 * @link     https://github.com/openapitools/openapi-generator
 */

/**
 * Adobe Experience Manager OSGI config (AEM) API
 *
 * Swagger AEM OSGI is an OpenAPI specification for Adobe Experience Manager (AEM) OSGI Configurations API
 *
 * OpenAPI spec version: 1.0.0-pre.0
 * Contact: opensource@shinesolutions.com
 * Generated by: https://github.com/openapitools/openapi-generator.git
 *
 */

/**
 * NOTE: This class is auto generated by the openapi generator program.
 * https://github.com/openapitools/openapi-generator
 * Do not edit the class manually.
 */

namespace OpenAPI\Server\Model;

use Symfony\Component\Validator\Constraints as Assert;
use JMS\Serializer\Annotation\Type;
use JMS\Serializer\Annotation\SerializedName;

/**
 * Class representing the ComDayCqReplicationContentStaticContentBuilderProperties model.
 *
 * @package OpenAPI\Server\Model
 * @author  OpenAPI Generator team
 */
class ComDayCqReplicationContentStaticContentBuilderProperties 
{
        /**
     * @var OpenAPI\Server\Model\ConfigNodePropertyString|null
     * @SerializedName("host")
     * @Assert\Type("OpenAPI\Server\Model\ConfigNodePropertyString")
     * @Type("OpenAPI\Server\Model\ConfigNodePropertyString")
     */
    protected $host;

    /**
     * @var OpenAPI\Server\Model\ConfigNodePropertyInteger|null
     * @SerializedName("port")
     * @Assert\Type("OpenAPI\Server\Model\ConfigNodePropertyInteger")
     * @Type("OpenAPI\Server\Model\ConfigNodePropertyInteger")
     */
    protected $port;

    /**
     * Constructor
     * @param mixed[] $data Associated array of property values initializing the model
     */
    public function __construct(array $data = null)
    {
        $this->host = isset($data['host']) ? $data['host'] : null;
        $this->port = isset($data['port']) ? $data['port'] : null;
    }

    /**
     * Gets host.
     *
     * @return OpenAPI\Server\Model\ConfigNodePropertyString|null
     */
    public function getHost()
    {
        return $this->host;
    }

    /**
     * Sets host.
     *
     * @param OpenAPI\Server\Model\ConfigNodePropertyString|null $host
     *
     * @return $this
     */
    public function setHost(ConfigNodePropertyString $host = null)
    {
        $this->host = $host;

        return $this;
    }

    /**
     * Gets port.
     *
     * @return OpenAPI\Server\Model\ConfigNodePropertyInteger|null
     */
    public function getPort()
    {
        return $this->port;
    }

    /**
     * Sets port.
     *
     * @param OpenAPI\Server\Model\ConfigNodePropertyInteger|null $port
     *
     * @return $this
     */
    public function setPort(ConfigNodePropertyInteger $port = null)
    {
        $this->port = $port;

        return $this;
    }
}


