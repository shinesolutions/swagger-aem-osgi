...
-- | display the config
instance P.Show AdobeExperienceManagerOSGIConfig(AEM)Config where
  show c =
    T.printf
...
-- | constructs a default AdobeExperienceManagerOSGIConfig(AEM)Config
--
-- configHost:
--
-- @http://localhost@
...
-- configUserAgent:
--
-- @"adobe-experience-manager-osgi-config-(aem)/0.1.0.0"@
--
newConfig :: IO AdobeExperienceManagerOSGIConfig(AEM)Config
newConfig = do
    logCxt <- initLogContext
    return $ AdobeExperienceManagerOSGIConfig(AEM)Config
        { configHost = "http://localhost"
        , configUserAgent = "adobe-experience-manager-osgi-config-(aem)/0.1.0.0"
...
addAuthMethod config@AdobeExperienceManagerOSGIConfig(AEM)Config {configAuthMethods = as} a =
  config { configAuthMethods = AnyAuthMethod a : as}

-- | updates the config to use stdout logging
withStdoutLogging :: AdobeExperienceManagerOSGIConfig(AEM)Config -> IO AdobeExperienceManagerOSGIConfig(AEM)Config
...
withStderrLogging :: AdobeExperienceManagerOSGIConfig(AEM)Config -> IO AdobeExperienceManagerOSGIConfig(AEM)Config
...
data AdobeExperienceManagerOSGIConfig(AEM)Request req contentType res accept = AdobeExperienceManagerOSGIConfig(AEM)Request
  { rMethod  :: NH.Method   -- ^ Method of AdobeExperienceManagerOSGIConfig(AEM)Request
  , rUrlPath :: [BCL.ByteString] -- ^ Endpoint of AdobeExperienceManagerOSGIConfig(AEM)Request
  , rParams   :: Params -- ^ params of AdobeExperienceManagerOSGIConfig(AEM)Request
  , rAuthTypes :: [P.TypeRep] -- ^ types of auth methods
  }
  deriving (P.Show)

-- | 'rMethod' Lens
rMethodL :: Lens_' (AdobeExperienceManagerOSGIConfig(AEM)Request req contentType res accept) NH.Method
rMethodL f AdobeExperienceManagerOSGIConfig(AEM)Request{..} = (\rMethod -> AdobeExperienceManagerOSGIConfig(AEM)Request { rMethod, ..} ) <$> f rMethod
{-# INLINE rMethodL #-}

-- | 'rUrlPath' Lens
rUrlPathL :: Lens_' (AdobeExperienceManagerOSGIConfig(AEM)Request req contentType res accept) [BCL.ByteString]
rUrlPathL f AdobeExperienceManagerOSGIConfig(AEM)Request{..} = (\rUrlPath -> AdobeExperienceManagerOSGIConfig(AEM)Request { rUrlPath, ..} ) <$> f rUrlPath
{-# INLINE rUrlPathL #-}

-- | 'rParams' Lens
rParamsL :: Lens_' (AdobeExperienceManagerOSGIConfig(AEM)Request req contentType res accept) Params
rParamsL f AdobeExperienceManagerOSGIConfig(AEM)Request{..} = (\rParams -> AdobeExperienceManagerOSGIConfig(AEM)Request { rParams, ..} ) <$> f rParams
{-# INLINE rParamsL #-}

-- | 'rParams' Lens
rAuthTypesL :: Lens_' (AdobeExperienceManagerOSGIConfig(AEM)Request req contentType res accept) [P.TypeRep]
rAuthTypesL f AdobeExperienceManagerOSGIConfig(AEM)Request{..} = (\rAuthTypes -> AdobeExperienceManagerOSGIConfig(AEM)Request { rAuthTypes, ..} ) <$> f rAuthTypes
{-# INLINE rAuthTypesL #-}
...
class HasBodyParam req param where
  setBodyParam :: forall contentType res accept. (Consumes req contentType, MimeRender contentType param) => AdobeExperienceManagerOSGIConfig(AEM)Request req contentType res accept -> param -> AdobeExperienceManagerOSGIConfig(AEM)Request req contentType res accept
  setBodyParam req xs =
...
class HasOptionalParam req param where
  ...
  applyOptionalParam :: AdobeExperienceManagerOSGIConfig(AEM)Request req contentType res accept -> param -> AdobeExperienceManagerOSGIConfig(AEM)Request req contentType res accept
  applyOptionalParam = (-&-)
  {-# INLINE applyOptionalParam #-}

  -- | infix operator \/ alias for 'addOptionalParam'
  (-&-) :: AdobeExperienceManagerOSGIConfig(AEM)Request req contentType res accept -> param -> AdobeExperienceManagerOSGIConfig(AEM)Request req contentType res accept
  (-&-) = applyOptionalParam
  {-# INLINE (-&-) #-}
...
class P.Typeable a =>
  ...
  applyAuthMethod
    ...
    -> IO (AdobeExperienceManagerOSGIConfig(AEM)Request req contentType res accept)
...
_applyAuthMethods
  ...
  -> AdobeExperienceManagerOSGIConfig(AEM)Config
  -> IO (AdobeExperienceManagerOSGIConfig(AEM)Request req contentType res accept)
_applyAuthMethods req config@(AdobeExperienceManagerOSGIConfig(AEM)Config {configAuthMethods = as}) =
  foldlM go req as
  where
