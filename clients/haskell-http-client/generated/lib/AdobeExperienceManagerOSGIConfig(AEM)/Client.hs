...
module AdobeExperienceManagerOSGIConfig(AEM).Client where
...
dispatchLbs
  ...
  -> AdobeExperienceManagerOSGIConfig(AEM)Config -- ^ config
  -> AdobeExperienceManagerOSGIConfig(AEM)Request req contentType res accept -- ^ request
...
runConfigLog
  :: P.MonadIO m
  => AdobeExperienceManagerOSGIConfig(AEM)Config -> LogExec m
runConfigLog config = configLogExecWithContext config (configLogContext config)

-- | Run a block using the configured logger instance (logs exceptions)
runConfigLogWithExceptions
  :: (E.MonadCatch m, P.MonadIO m)
  => T.Text -> AdobeExperienceManagerOSGIConfig(AEM)Config -> LogExec m
