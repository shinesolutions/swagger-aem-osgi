## OpenAPI Auto-Generated [http-client](https://www.stackage.org/lts-10.0/package/http-client-0.5.7.1) Bindings to `Adobe Experience Manager OSGI config (AEM) API`

The library in `lib` provides auto-generated-from-OpenAPI [http-client](https://www.stackage.org/lts-10.0/package/http-client-0.5.7.1) bindings to the Adobe Experience Manager OSGI config (AEM) API API.
...
| baseModule                      | Set the base module namespace                                                                                                 |          | AdobeExperienceManagerOSGIConfig(AEM)                      |
| cabalPackage                    | Set the cabal package name, which consists of one or more alphanumeric words separated by hyphens                             |          | adobe-experience-manager-osgi-config-(aem)                    |
...
| configType                      | Set the name of the type used for configuration                                                                               |          | AdobeExperienceManagerOSGIConfig(AEM)Config                      |
...
| requestType                     | Set the name of the type used to generate requests                                                                            |          | AdobeExperienceManagerOSGIConfig(AEM)Request                     |
...
| MODULE              | NOTES                                               |
| ------------------- | --------------------------------------------------- |
| AdobeExperienceManagerOSGIConfig(AEM).Client    | use the "dispatch" functions to send requests       |
| AdobeExperienceManagerOSGIConfig(AEM).Core      | core funcions, config and request types             |
| AdobeExperienceManagerOSGIConfig(AEM).API       | construct api requests                              |
| AdobeExperienceManagerOSGIConfig(AEM).Model     | describes api models                                |
| AdobeExperienceManagerOSGIConfig(AEM).MimeTypes | encoding/decoding MIME types (content-types/accept) |
| AdobeExperienceManagerOSGIConfig(AEM).ModelLens | lenses for model fields                             |
| AdobeExperienceManagerOSGIConfig(AEM).Logging   | logging functions and utils                         |
