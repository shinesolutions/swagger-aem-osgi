#include <map>
#include <cstdlib>
#include <glib-object.h>
#include <json-glib/json-glib.h>
#include "Helpers.h"


#include "ComDayCqReportingImplRLogAnalyzerProperties.h"

using namespace std;
using namespace Tizen::ArtikCloud;

ComDayCqReportingImplRLogAnalyzerProperties::ComDayCqReportingImplRLogAnalyzerProperties()
{
	//__init();
}

ComDayCqReportingImplRLogAnalyzerProperties::~ComDayCqReportingImplRLogAnalyzerProperties()
{
	//__cleanup();
}

void
ComDayCqReportingImplRLogAnalyzerProperties::__init()
{
	//requestlogoutput = new ConfigNodePropertyString();
}

void
ComDayCqReportingImplRLogAnalyzerProperties::__cleanup()
{
	//if(requestlogoutput != NULL) {
	//
	//delete requestlogoutput;
	//requestlogoutput = NULL;
	//}
	//
}

void
ComDayCqReportingImplRLogAnalyzerProperties::fromJson(char* jsonStr)
{
	JsonObject *pJsonObject = json_node_get_object(json_from_string(jsonStr,NULL));
	JsonNode *node;
	const gchar *requestlogoutputKey = "request.log.output";
	node = json_object_get_member(pJsonObject, requestlogoutputKey);
	if (node !=NULL) {
	

		if (isprimitive("ConfigNodePropertyString")) {
			jsonToValue(&requestlogoutput, node, "ConfigNodePropertyString", "ConfigNodePropertyString");
		} else {
			
			ConfigNodePropertyString* obj = static_cast<ConfigNodePropertyString*> (&requestlogoutput);
			obj->fromJson(json_to_string(node, false));
			
		}
	}
}

ComDayCqReportingImplRLogAnalyzerProperties::ComDayCqReportingImplRLogAnalyzerProperties(char* json)
{
	this->fromJson(json);
}

char*
ComDayCqReportingImplRLogAnalyzerProperties::toJson()
{
	JsonObject *pJsonObject = json_object_new();
	JsonNode *node;
	if (isprimitive("ConfigNodePropertyString")) {
		ConfigNodePropertyString obj = getRequestlogoutput();
		node = converttoJson(&obj, "ConfigNodePropertyString", "");
	}
	else {
		
		ConfigNodePropertyString obj = static_cast<ConfigNodePropertyString> (getRequestlogoutput());
		GError *mygerror;
		mygerror = NULL;
		node = json_from_string(obj.toJson(), &mygerror);
		
	}
	const gchar *requestlogoutputKey = "request.log.output";
	json_object_set_member(pJsonObject, requestlogoutputKey, node);
	node = json_node_alloc();
	json_node_init(node, JSON_NODE_OBJECT);
	json_node_take_object(node, pJsonObject);
	char * ret = json_to_string(node, false);
	json_node_free(node);
	return ret;
}

ConfigNodePropertyString
ComDayCqReportingImplRLogAnalyzerProperties::getRequestlogoutput()
{
	return requestlogoutput;
}

void
ComDayCqReportingImplRLogAnalyzerProperties::setRequestlogoutput(ConfigNodePropertyString  requestlogoutput)
{
	this->requestlogoutput = requestlogoutput;
}


